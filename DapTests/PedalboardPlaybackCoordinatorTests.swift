...
private final class PlaybackPlayerDouble: PedalPlaying {
    ...
    private let emitsStopReasonOnFailure: DapSynthStopReason?

    init(failOnPlayNumbers: Set<Int> = [], emitsStopReasonOnFailure: DapSynthStopReason? = nil) {
    ...
    func emit(_ reason: DapSynthStopReason) {
        isPlaying = false
        stopHandler?(reason)
    }
}
