...
struct ProgressivePedalMetadataTests {
    @Test func essentialResultPersistsFallbackBeforeSemanticMetadataCompletes() async throws {
        ...
        let model = PhotoPedalViewModel(store: store, pipeline: pipeline(generator: generator))
        ...
        #expect(model.pedal?.name == "Photo Pedal")
    ...
    @Test func successfulEnrichmentUpdatesOnlyMetadataForSameRecord() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = PedalStore(directory: directory)
        let original = pedal(name: "Photo Pedal", description: "A photo-generated sound pedal.")
    ...
    @Test func invalidOrFailedEnrichmentKeepsPersistedFallback() async throws {
        ...
        for failure in failures {
            ...
            let saved = try #require(store.loadLatest())
            #expect(saved.pedal.name == "Photo Pedal")
    ...
    @Test func missingCorruptAndDeletedRecordsAreNotRecreatedByMetadataUpdate() throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = PedalStore(directory: directory)
        let item = pedal(name: "Photo Pedal", description: "A photo-generated sound pedal.")
    ...
    @Test func galleryUpdatesExistingItemWithoutChangingOrderOrCount() throws {
        ...
        let newer = pedal(name: "Photo Pedal", description: "A photo-generated sound pedal.", date: Date(timeIntervalSince1970: 2))
    ...
    private func pipeline(generator: any PedalMetadataGenerating) -> PhotoPedalPipeline {
        PhotoPedalPipeline(
