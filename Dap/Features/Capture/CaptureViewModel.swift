...
final class DapViewModel {
    ...
    private let pipeline: DapPipeline
    private let synth = DapSynth()
    ...
    convenience init() { self.init(store: .shared, pipeline: DapPipeline()) }
    convenience init(metadataUpdateHandler: @escaping (StoredPedal) -> Void) {
        self.init(store: .shared, pipeline: DapPipeline(), metadataUpdateHandler: metadataUpdateHandler)
    }
    convenience init(store: PedalStore, metadataUpdateHandler: @escaping (StoredPedal) -> Void = { _ in }) {
        self.init(store: store, pipeline: DapPipeline(), metadataUpdateHandler: metadataUpdateHandler)
    }
    init(
        store: PedalStore = .shared,
        pipeline: DapPipeline,
